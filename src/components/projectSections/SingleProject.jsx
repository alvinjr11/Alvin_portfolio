...
const SingleProject = ({ name, year, align, description, image, link }) => {
    return (
        <motion.div
       ...
       className={`flex w-full sm:flex-col-reverse items-center gap-8
         ${align === 'left' ? 'md:flex-row' : 'md:flex-row-reverse'} justify-end`}>
            <div>
                <h2 className={`md:text-3xl sm:text-2xl text-orange sm:text-center ${align === "left" ? "md:text-right" : "md:text-left"
                        }`}>{name}</h2>
                ...
                <p
                    ...
                    className={` text-white mt-2 sm:text-center ${align === "left" ? "md:text-right" : "md:text-left"
